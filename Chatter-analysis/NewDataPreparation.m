    ...
    methods 
        ...
        function [obj] = DataPreparation(file_N,accelMatrix,fs,file_num,folder_num)
            if nargin > 0
                obj.fs = fs;
                obj.N = file_N;
                obj.TimeTableAccel = timetable(obj.TimeVector(), accelMatrix[:,2:5],...
                    'VariableNames', ['time','zt','z','x','y']);
                obj.TimeTableAcoustic = timetable(obj.TimeVector(), accelMatrix[:,1], ...
                     'VariableNames' , ['time','ae']);
        ...
        function [obj] = TimeVector(obj)
            ...
            % fs = sampling rate
            obj.N = length(obj.x);
            obj.tvec = seconds(0:1/obj.fs:(obj.N -1)/obj.fs)';
        end
        ...
        function [obj] = PreProcess(obj)
            % Makes sure all data point are separated by an equal timeStep.
            obj.TimeTableObject = resample(obj.TimeTableObject, 1,1)
